...
First, look for a dedicated test plan file — typically `TEST-PLAN.md`, `test-plan.md`, or `TESTING.md` in the project. If found, use it as the primary source.

If **no dedicated test plan file** is found, search freely across all project documentation (markdown files, READMEs, deployment guides, conversion docs) for any sections containing test steps, verification instructions, or API/endpoint examples. Use whatever sources you find to assemble the full set of test steps.
...
Write to `/tmp/e2e-results.yaml` following that schema.
